...
export type Action = Map$Type$Payload<
	{
		...
		[ActionType.updateLineTime]: { index: number; time: number, audioTime: number };
	},
	ActionType
>;
...
const reducer = (state: IState, action: Action): IState => {
	switch (action.type) {
		...
		case ActionType.updateLineTime: {
			const {index, time, audioTime} = action.payload;
			if (index < 0 || index >= state.lyric.length || state.lyric[index].time === time) {
				return state;
			}
			const newLyric = state.lyric.slice();
			newLyric[index] = {text: newLyric[index].text, time};

			let newCurrentIndex = state.currentIndex;

			for (let i = 0; i < newLyric.length; i++) {
				const currentLine = newLyric[i];
				if (!currentLine.time) {
					continue;
				}
				const nextLine = newLyric[i+1];
				if (!nextLine) {
					newCurrentIndex = i;
					break;
				}
				...
				if (currentLine.time <= audioTime && audioTime < nextLine.time) {
					newCurrentIndex = i;
					break;
				}
			}
			return {...state, lyric: newLyric, currentIndex: newCurrentIndex};
		}
	}
};
